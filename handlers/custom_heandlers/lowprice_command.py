...
@bot.callback_query_handler(
    ...
    state=HotelLowPriceState.check_in
)
    ...
    if action == "DAY":
        ...
        with bot.retrieve_data(call.message.chat.id, call.message.chat.id) as hotels_data:
            if datetime.date(int(year), int(month), int(day)) >= datetime.date.today():
                ...
                bot.set_state(call.message.chat.id, HotelLowPriceState.check_out)
            else:
...
@bot.callback_query_handler(
    func=lambda call: call.data.startswith(calendar_1_callback.prefix),
    state=HotelLowPriceState.check_out
)
def callback_inline(call: CallbackQuery):
    """
    ...
    if action == "DAY":
        ...
        with bot.retrieve_data(call.message.chat.id, call.message.chat.id) as hotels_data:
            if datetime.date(int(year), int(month), int(day)) >= hotels_data['check_in']:
                ...
                bot.set_state(call.message.chat.id, HotelLowPriceState.count_hotel)
            else:
...
@bot.callback_query_handler(func=None, state=HotelLowPriceState.count_hotel)
...
@bot.callback_query_handler(func=None, state=HotelLowPriceState.count_photos)
def count_photos(call) -> None:
    ...
    bot.set_state(call.message.chat.id, HotelLowPriceState.info, call.message.chat.id)
    get_search_results(call.message, count_photos=int(call.data), call_chat_id=call.message.chat.id)
